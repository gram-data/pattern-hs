...
runMatch opts = do
  ...
  case (Gram.fromGram patternStr, Gram.fromGram dataStr) of
    (Left err, _) -> do
      Output.formatError (matchFormat opts) ("Pattern parse error: " ++ show err)
      return (ExitFailure 1)
    (_, Left err) -> do
      Output.formatError (matchFormat opts) ("Data parse error: " ++ show err)
      return (ExitFailure 1)
    (Right pattern, Right dataPattern) -> do
      ...
      if Pattern.matches pattern dataPattern
        then do
          Output.formatOutput (matchFormat opts) dataPattern
          return ExitSuccess
        else do
          Output.formatError (matchFormat opts) "Pattern does not match data"
