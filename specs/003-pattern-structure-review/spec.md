...
As a developer using Pattern, I need clear and consistent definitions of pattern variants (atomic patterns, nodes, relationships, subgraphs, paths) so that I understand when and how to use each variant.
...
- **FR-004**: System MUST provide clear definitions for all pattern variants (atomic patterns, nodes, relationships, subgraphs, paths) with explicit implementation status
...
- **Pattern Variants**: Structural classifications of patterns based on their element structure (atomic patterns, nodes, relationships, subgraphs, paths) that can be interpreted through different graph views. Variants are determined by the structure of elements, and views provide different semantic interpretations of those structures.
